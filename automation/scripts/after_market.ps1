...
if ($AllExist) {
    ...
    # === 每日：更新法人模式追蹤器 ===
    Write-Output "" | Tee-Object -FilePath $LogFile -Append
    Write-Output "[模式追蹤] 更新法人賺錢模式..." | Tee-Object -FilePath $LogFile -Append
    python "$ProjectDir\scripts\institutional_pattern_tracker.py" 2>&1 | Tee-Object -FilePath $LogFile -Append
