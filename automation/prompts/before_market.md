...
⚠️ 持倉危急（距停損 < 5% 的持有中股票，必須列出）
每檔格式：
  代碼 名稱 DayN/10 距停損X% 🔴｜停損 {stop_loss}元（必須列出絕對價格）
...
- **持倉危急區塊必須標出停損絕對價格**（不能只寫「距停損X%」，要寫「停損 50.5元」）
- 催化x營收有⚠️的產業要獨立列出
- PTT 有熱門個股才列，沒有就省略
...
5. **🔴 禁止執行 `python3 scripts/notify_line.py`**，LINE 推送由排程腳本統一處理，Claude 執行會造成重複發送
