...
**Step 3: Track B分析（第三優先）** 🚨 **防遺漏關鍵**
**🔥 Track B強制內容**：
1. **市場發現TOP5分析** - 盤前沒推薦但大漲的股票
2. **遺漏機會檢討** - 為什麼盤前沒推薦？法人數據問題？產業邏輯不足？
3. **策略驗證** - 推薦成功vs避開成功的驗證
4. **產業共振分析** - 同產業股票表現（如記憶體族群）
5. **新標的發現** - 盤中異動但標註[僅記錄、不追高]
...
**核心規則提醒**：
- ✅ **Track A + Track B 同等重要**
- ✅ **市場發現 > 只看推薦股**
- ❌ 絕不能只做Track A，跳過Track B
- ❌ 絕不能沒有「遺漏機會檢討」
- ❌ 絕不能沒有「策略驗證」分析
...
**最後更新**：2025-12-03
**版本**：v5.0（歷史驗證強制化+系統流程重大改版）
**重大更新**：
- 🚨 **歷史驗證強制化**：盤前分析必須先查tracking.json
- 🚨 **流程完全重構**：歷史驗證 → 調整推薦 → 法人數據 → 評分
- 🚨 **準確率大幅改善**：從33.3%(12/02)通過歷史驗證避開失敗模式
- 🚨 **防止重複失敗**：建立失敗教訓資料庫、避開相同錯誤
- 🆕 **新增工具**：fetch_us_asia_markets.py國際市場數據
- 🆕 **新增文件**：HISTORICAL_VALIDATION_CHECKLIST.md
