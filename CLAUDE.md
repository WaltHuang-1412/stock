...
### 🔴 Step 5.5: 催化預埋掃描（強制）🆕 v7.9.3

**目的**：自動偵測「法人多日佈局 + 股價尚未反映」的預埋機會，提前 1-2 天進場

**背景**：催化劑爆發日（如 Micron+9.6%）法人往往已在出貨（Level 4），散戶追進去等於接刀。本步驟在催化爆發前找到法人正在佈局的股票。
...
**驗證**：
- ✅ 必須生成 `data/YYYY-MM-DD/catalyst_preposition_scan.json`
- ❌ **如果文件不存在 = 步驟未執行 = 禁止繼續**
...
腳本自動回看近 7 個交易日的 `institutional_top50.json`，找出：
1. 多日出現在法人買超 TOP50 的股票
2. 5 日漲幅 < 5%（股價尚未充分反映）
3. 呼叫 `chip_analysis.py` 取動能+連買天數
4. 排除累計賣超、10天中≥7天賣超的股票
5. 依動能+連買天數分級
...
| 層級 | 條件 | 倉位 | 信心 |
|------|------|------|------|
| **🔥 L3 佈局完成** | 連3+天買超 + 動能<-30% + 漲幅<5% | **15-20%** | 高 |
| **🟢 L2 早期佈局** | 連2+天買超（或≥6天買超）+ 動能<+50% + 漲幅<5% | **5-10%** | 中 |
| **🟡 L1 態度轉變** | 在TOP50 + 漲幅<3% + 動能<+100% | **觀察** | 低 |
| **⚠️ 追高排除** | 動能>+100% | **不推薦** | - |
...
|---------|---------------|--------------|
| **L3 佈局完成** | ✅ 自動進入候選池，**優先評分** | 動能加分+15（佈局完成）+「🔥 預埋L3」標籤 |
| **L2 早期佈局** | ✅ 進入候選池 | 動能加分+10（佈局中）+「🟢 預埋L2」標籤 |
| **L1 態度轉變** | ✅ 進入觀察名單 | 不額外加分，但提示「法人態度轉變中」 |
| **追高排除** | ❌ 不進入候選池 | 不評分（除非超強催化覆寫） |

**🔴 L3 股票必須進入 Step 7 評分**：即使不在當天 TOP20，L3 佈局完成的股票也要強制評分（類似 TOP20 強制評分規則）。
...
### 🔥 L3 佈局完成（N 檔）→ 正常倉位 15-20%
- 中鼎(9933)：6天TOP50+連9天買超+動能-34.1%+漲幅僅+1.9%
- 中華電(2412)：4天TOP50+連10天買超+動能-48.4%+漲幅僅+1.5%

### 🟢 L2 早期佈局（N 檔）→ 小倉位 5-10%
- 宏碁(2353)：3天TOP50+動能-59.9%+漲幅僅+4.7%

### 🟡 L1 態度轉變（N 檔）→ 僅觀察
- 友達(2409)：3天TOP50+動能-191.7%+4買6賣

### ⚠️ 追高排除
- 華邦電(2344)：動能+275%已追高，法人可能正在出貨
- 長榮(2603)：動能+500%已追高
```
...
| 掃描結果 | 五維度實際結果 | 驗證 |
|---------|-------------|------|
| 中鼎 L3 → 優先推薦 | 77分推薦 ✅ | 掃描與五維度一致 |
| 中華電 L3 → 優先推薦 | 66分可考慮 ✅ | 掃描提前確認法人佈局 |
...
**🆕 綜合檢視原則（v7.9.2）**：
評分時須綜合檢視三大法人合計、外資、投信三者的買賣天數與累計方向，不得僅以合計數據判斷。
...
**🆕 與預埋掃描（Step 5.5）的整合**（v7.9.3）：

預埋掃描的 L3/L2 分級與動能評分表自動對齊：
- 預埋 L3（動能<-30%）→ 動能評分已含+15分+強制評分
- 預埋 L2（動能<+50%）→ 動能評分已含+10~0分
- 預埋「追高排除」→ 動能>+100% 排除規則已涵蓋

**🔴 L3 強制評分規則**：預埋掃描 L3 的股票，即使不在 TOP20，也必須進入五維度評分（與 TOP20 強制評分同等優先級）。原因：L3 佈局完成代表法人已連續多日買超且動能收斂，是最高信心的進場訊號。
...
**覆寫後處理**：
- 評分調整：**不扣分**（超強催化本身是動能爆發的原因，不重複懲罰。風險由倉位控制）
- 倉位上限：**5-10%**（⚠️ 動能高，小倉位控制風險）
- 標籤：標註「🔥 超強催化覆寫」
...
### 0. **catalyst_preposition_scan.py** - 催化預埋掃描 🆕 v7.9.3

**用途**：自動偵測法人多日佈局+股價未反映的預埋機會
...
**功能**：
1. 回看 N 天 `institutional_top50.json`，統計每檔股票多日出現次數+累計買超
2. 呼叫 `chip_analysis.py` 取動能+連買天數
3. 分級：L3（佈局完成）/ L2（早期佈局）/ L1（態度轉變）/ 追高排除
4. 輸出 `catalyst_preposition_scan.json`
