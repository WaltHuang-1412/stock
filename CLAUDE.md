...
cat data/tracking/tracking_2025-11-**.json  # 查看最新tracking
```

**Step 2: 分析成功延續性（第二優先）**
- 上次推薦哪些成功了？成功原因是什麼？
- 這些成功因素是否延續？（法人買超、產業邏輯、價格位置）
- **原則：已驗證成功的股票 > 未驗證的新推薦**
...
```python
# 必須查詢實際股價，不能憑印象
python3 -c "import yfinance as yf; print(yf.Ticker('2330.TW').history(period='1d')['Close'].iloc[-1])"
```
...
**版本**：v4.1（檔案結構整理+系統更新）
**今日更新**：
- 🆕 **檔案結構整理**：所有工具移至 scripts/ 目錄
- 🆕 **三套系統並存**：驗證+發現+追蹤系統說明
- 🆕 **準確率更新**：12/01最新50%（2/4）
- 🆕 **每日四份報告**：盤前+盤中+佈局偵測+盤後
**核心功能**：
- ✅ 歷史驗證優先（不只看當天）
- ✅ 五維度評分系統
- ✅ 防事後諸葛機制
