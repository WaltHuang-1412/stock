...
**🔴 盤中新增 L3/L4 邊緣值待複核（v8.3.2 新增，強制）**：

`reversal_alert.py` 的 Level 4 門檻為「賣超 > 5% 日均量」，其**分子為 T86 前一交易日賣超（盤中固定不變）、分母日均量在盤中隨當日累計成交量浮動** → 量縮股分母被壓低而假性跨過門檻、爆量股分母被墊高而假性降級。

| 情況 | 動作 |
|------|------|
| **盤前**已是 L3/L4（分母為完整前一交易日量） | 🛑 立即出場，不受本規則影響 |
| **盤中新增** L3/L4（盤前非 L3/L4）**且** 賣超佔日均量 **< 8%** | ⚠️ 列「待複核」**不立即出場**；盤後以完整日量重跑 `reversal_alert.py`，仍為 L3/L4 才於**次日開盤出場**，降級則維持追蹤並於盤後報告揭露 |
| **盤中新增** L3/L4 **且** 賣超佔日均量 ≥ 8%（或 >50K 張） | 🛑 立即出場（遠離門檻，非分母效應） |
| 同時觸及停損價或 2 日累計 -10% | 🛑 立即出場（其他鐵律優先，不受本規則保護） |

> 依據：2026-08-12 盤中三筆等級變動（2356 L4→L0、2867 L2→L4、3042 L0→L4）賣超張數完全未變，全部落在 5% 門檻 ±0.6pp 內；盤後以完整日量複核 2867 降為 L2（3.8%）、3042 與 2356 降為 L0。當日 2867／3042 因此被非必要強制出場。**本規則不放寬盤前 L4 的立即出場，只擋掉分母效應造成的邊緣誤觸發。**

**尾盤策略**：✅ 續抱 | 🛑 強制停損 | ⚠️ 待複核（僅限上表第 2 列）
...
**最後更新**：2026-08-12
**版本**：v8.3.2（盤中新增 L3/L4 邊緣值待複核；營收快取新鮮度檢查；settlement_checker 查詢失敗必須清零＋鐵律出場入帳價定為收盤價）
