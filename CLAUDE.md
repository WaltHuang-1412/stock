...
**時間軸**：
```
09:00前  盤前分析 → 時事40% + 法人40% + 技術20% → 預測6-10檔
12:30    盤中分析 → intraday_scanner.py → 尾盤策略
14:30後  盤後分析 → 驗證成敗 + 全市場法人 + 明日佈局
```

**核心原則**：
- **盤前**：全市場掃描、預測6-10檔+明確價位、產業邏輯清晰
- **盤中**：執行intraday_scanner.py、只分析預測股、給尾盤策略
- **盤後**：驗證盤中預測（主60%）、驗證盤前預測（次20%）、更新TRACKING.md

**品質檢查清單**：
- [ ] 盤前：全市場掃描？明確價位？產業邏輯？
- [ ] 盤中：執行scanner？只分析預測股？尾盤策略？
- [ ] 盤後：驗證盤中？驗證盤前？全市場法人？更新TRACKING？
