...
## 🚨 每次對話開始強制流程（2025/11/12新增）

**重要**：每次新對話的第一件事，必須執行以下檢查：

### 強制檢查清單
1. **讀取** `CONVERSATION_START_CHECKLIST.md`（對話開始檢查清單）
2. **讀取** `PREDICTION_ACCURACY_TRACKING.md` 最後3筆記錄（學習歷史教訓）
3. **讀取** `data/最新日期/after_market_analysis.md`（確認昨日預測）
4. **確認**今日需要驗證的預測項目
5. **記住**核心規則：
   - ✅ 盤中預測→隔日驗證（不是當日收盤）
   - ❌ 不事後諸葛、不追高推薦、不主觀選股
   - 📊 買賣比需配合連續買超、爆量、產業邏輯綜合判斷
...
**案例**：晟銘電（2025/11/11-12）
- 11/11盤中預測+1.09%「吸貨中」
- 11/11收盤-0.36% → 我判定失敗 ❌
- 11/12收盤+5.82% → 用戶糾正：要隔日驗證 ✅

---

## ⚠️ 重要警告：預測準確率極低（<10%）→已改善至70%
**歷史教訓**：2025/10/01 預測12項只對1項，準確率8.3%
**最新成果**：2025/11/11 整體準確率70%（7/10）
- 詳見 PREDICTION_ACCURACY_TRACKING.md 追蹤記錄
- **關鍵改善**：盤中預測改用「隔日驗證」、新增買賣比指標
