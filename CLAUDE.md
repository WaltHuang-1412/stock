...
>
> 同日盤後**提出但未定案**（待 08/24 驗證）：「`settlement_checker` 收盤序列日期對齊檢查」—— 2867 三商壽價格序列停於 08-19（Yahoo 08-20 volume=0、08-21 close=None，08-20 T86 亦查無），腳本以 08-19 收盤 9.70 當「現價」輸出 D2/10 且**無任何警告**（腳本沒有失敗，v8.3.2 的「查詢失敗清零」抓不到）。因僅單一未確認樣本（疑似暫停交易），**先揭露不立法**，08/24 盤前確認交易狀態後再議。
