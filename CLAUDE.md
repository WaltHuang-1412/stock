...
| 階段 | 最低要求 |
|------|---------|
| **盤前** | 雙入口掃描(TOP30+時事展開)、推薦 6-8檔、產業分散≥4個、單一產業≤50%、建檔 .md + tracking.json |
...
| **Step 3** | 雙入口掃描+評分 | 30分鐘 | 入口A(TOP30)+入口B(時事展開)+五維度評分 |
...
### Step 3 整合內容（30分鐘）
雖然顯示為 1 個步驟，但仍需執行：
1. **入口 A**：TOP30 輸出（買超30+賣超30）+ 時事驗證
2. **入口 B**：時事催化展開 → 受惠產業代表股 → 查法人態度
3. 合併去重 + 統一五維度評分（達標≥75分推薦）
...
[
  ...
  {"content": "Step 3: 雙入口掃描(TOP30+時事展開)+五維度評分", "status": "pending", "activeForm": "執行選股"},
...
**Step 3: 🔥 雙入口選股（v6.1 重構）** 🔴 **核心步驟**

**🆕 Step 3.0: 催化劑強度判定（v6.1 新增）** 🔥

**⚠️ 血淚教訓（2026-01-28）**：
- 問題：費半+4.14%（超強催化），記憶體股南亞科+6.86%、華邦電+9.96%、旺宏+9.97%
- 盲點：固定規則「5日漲幅>10%絕對不推薦」在超強催化日排除了最大漲幅股票
- 結論：**催化越強 → 規則越應放寬，否則會系統性錯過最佳機會**

**🔴 催化劑強度分級表（盤前第一步判定）**：

| 等級 | 條件 | 產業集中上限 | 排除規則調整 | 適用場景 |
|------|------|------------|-------------|---------|
| ⚪ **一般** | 費半 <1% | ≤50%（原規則） | 所有規則照舊 | 大多數交易日 |
| 🟡 **強催化** | 費半 1-3% | ≤60% | 催化產業：>5%正常評分、>10%標註風險但不排除 | 費半明顯上漲 |
| 🔴 **超強催化** | 費半 >3% | ≤70% | 催化產業：>5%正常評分、>10%可小倉位5% | 費半大漲（如+4.14%） |

**⚠️ 重要限制**：
- 放寬規則**僅適用於催化相關產業**（如費半大漲 → 半導體相關）
- **非催化產業維持原規則**（如費半大漲時，金融股規則不變）
- 超強催化日的高漲幅股票，倉位上限5%、停損-3%（快進快出）

**判定流程**：
```
1. 查 Step 0 國際時事數據 → 費半漲跌幅
2. 判定催化等級：一般 / 強催化 / 超強催化
3. 確認催化相關產業（費半→半導體5次產業、輝達→AI伺服器等）
4. 後續 Step 3.1-3.4 依催化等級調整規則
```

**範例**：
```
2026-01-28：費半+4.14% → 🔴 超強催化
→ 催化產業：半導體（晶圓代工/載板/記憶體/封測/設備）
→ 產業集中上限：70%（可推7檔中5檔半導體）
→ 記憶體股即使5日漲幅8%，仍可正常評分推薦
→ 金融股維持原規則（≤50%、>10%不推薦）
```
...
**⚠️ 重大教訓**：
- （2026-01-22）用「時事→產業」限縮視野，漏掉正達(3149)
- （2026-01-28）TOP30 是法人入口，記憶體股南亞科不在 TOP30 就完全消失
- **根本原因**：昨日法人數據無法預知昨晚時事，只靠法人入口必有盲區
- **結論**：法人入口 + 時事入口雙軌並行，合併後統一評分

**🔴 正確流程（v6.0 雙入口）**：
```
入口 A（法人）: TOP30 法人買賣超 → 每檔查時事催化
入口 B（時事）: 今日催化劑 → 展開受惠產業 → 拉代表股 → 查法人態度
                        ↓
              合併去重 → 統一五維度評分 → ≥75分推薦
```
...
對 TOP30 每一檔，快速檢查有無時事催化劑：
| 標註 | 意義 | 後續動作 |
|------|------|---------|
| ⭐ 有重大催化劑 | 個股新聞利多 | **必須**五維度評分 |
| 🔥 產業利多 | 符合時事→產業對照 | 五維度評分 |
| ✅ 法人買+還沒漲 | 佈局機會 | 五維度評分 |
| ⚠️ 已大漲 | 5日漲幅>5% | 標註追高風險 |
| -- | 無特別催化 | 可跳過 |
...
**🔥 Step 3.2: 入口 B — 時事催化展開（強制）** 🆕 2026-01-28

**⚠️ 血淚教訓（2026-01-28）**：
- 問題：費半+4.14%但記憶體股南亞科(2408)完全沒被掃到
- 原因：南亞科不在法人 TOP30，而時事→產業對照表只是「輔助」沒真的拉股票查
- 後果：半導體5個次產業只查了3個（晶圓代工、AI伺服器、封裝），漏掉載板和記憶體

**🔴 強制執行流程**：

1. **從 Step 0 的時事結果，判斷今日催化方向**
2. **查「時事→產業→代表股」對照表，列出所有受惠代表股**
3. **扣除已在 TOP30 出現的股票（避免重複）**
4. **對剩餘股票查法人數據 + 即時股價**
```bash
# 對時事展開的代表股，查法人態度
python3 scripts/chip_analysis.py [代表股代號1] [代表股代號2] ...
```
5. **合併入口 A 結果，進入統一評分**

**時事→產業→代表股 完整對照表**：

| 時事訊號 | 觸發條件 | 受惠產業 | 代表股（必查） |
|---------|---------|---------|---------------|
...
| 輝達/AMD大漲 | >3% | AI伺服器 | 廣達(2382)、緯創(3231)、鴻海(2317)、英業達(2356) |
| | | 散熱 | 雙鴻(3324)、超眾(6230) |
| | | 電源供應 | 康舒(6282)、台達電(2308) |
| | | 網通/交換器 | 智邦(2345)、啟碁(6285) |
| 台積電ADR大漲 | >2% | CoWoS供應鏈 | 欣興(3037)、日月光(3711)、力成(6239) |
| 油價大漲 | >2% | 塑化（利多） | 台塑(1301)、南亞(1303)、台化(1326) |
| | | 航空（利空） | 華航(2610)、長榮航(2618) → 標註油價逆風 |
| 油價大跌 | <-2% | 航空（利多） | 華航(2610)、長榮航(2618) |
| | | 塑化（利空） | 台塑(1301)、南亞(1303) → 標註油價逆風 |
| 記憶體漲價 | 新聞 | 記憶體 | 南亞科(2408)、華邦電(2344)、旺宏(2337) |
| 面板漲價 | 新聞 | 面板 | 群創(3481)、友達(2409) |
| 美元走弱 | DXY<100 | 壽險（利多） | 國泰金(2882)、富邦金(2881)、新光金(2888) |
| 央行升息 | 新聞 | 銀行（利多） | 彰銀(2801)、華南金(2880)、第一金(2892) |
| 成交量暴增 | >3500億 | 證券（利多） | 元大金(2885)、凱基金(2883) |
| KOSPI大漲 | >2% | 半導體連動 | 同「費半大漲」展開 |

**🔥 入口 B 輸出格式**：
```markdown
## 入口 B：時事展開（不在 TOP30 的受惠股）

今日催化：費半+4.14%、輝達+5.87%、日月光ADR+6.19%

| 代號 | 名稱 | 受惠催化 | 在TOP30? | 法人態度 | 5日漲幅 | 後續 |
|------|------|---------|:-------:|---------|--------|------|
| 2408 | 南亞科 | 費半→記憶體 | ❌ | 查chip_analysis | +X% | 評分 |
| 3037 | 欣興 | 費半→載板 | ❌ | 查chip_analysis | +X% | 評分 |
| 3711 | 日月光 | ADR+6.19% | ❌ | 查chip_analysis | +X% | 評分 |
| ... | ... | ... | ... | ... | ... | ... |
（扣除已在 TOP30 的股票）
```

**❌ 絕對禁止**：
1. ❌ 時事催化明確但只查 TOP30 就結束
2. ❌ 對照表列了代表股但沒實際查法人數據
3. ❌ 費半>1% 但沒展開 5 個半導體次產業
4. ❌ 入口 B 的股票跳過評分直接推薦或直接忽略

---

**🔥 Step 3.3: 合併去重 + 統一評分**

將入口 A（TOP30）和入口 B（時事展開）的候選股合併：
1. **去重**：同一檔股票只評一次
2. **統一五維度評分**：不管從哪個入口進來，標準相同
3. **標註來源**：在推薦中標明「TOP30入口」或「時事入口」

```markdown
| 股票 | 入口來源 | 五維度總分 | 推薦? |
|------|---------|-----------|------|
| 廣達 | A(TOP30) + B(輝達催化) | 86 | ✅ |
| 南亞科 | B(費半→記憶體) | 7X | 依分數 |
| 力成 | A(TOP30) + B(日月光ADR) | 78 | ✅ |
```

---

**🔥 Step 3.4: 時事→產業對照表（驗證用）**

**用途**：最後驗證「時事利多的產業是否都有覆蓋到」

**⚠️ 重要**：
- 入口 A + 入口 B 合併後，對照此表檢查是否有遺漏
- 如果某個受惠產業完全沒有候選股 → 回頭查代表股
- 這是最後防線，不是起點
...
| 檢查項目 | 說明 | 範例 |
|---------|------|------|
| 重大合作 | 戰略聯盟、技術授權 | 正達+康寧合作 |
| 新產品線 | 新事業、新應用 | HDD玻璃碟盤 |
| 大額訂單 | 客戶訂單、擴產 | 獨家供應合約 |
| 法說會/財報 | 業績展望 | 營收創高 |
...
**催化相關產業（強催化/超強催化日）— 放寬規則** 🆕：
| 催化等級 | 5日漲幅 > 10% | 5日漲幅 5-10% | 5日漲幅 < 5% |
|---------|--------------|--------------|-------------|
| 🟡 強催化 | ⚠️ 標註風險，不絕對排除（倉位≤5%） | 正常評分（不扣分） | ✅ 優先推薦 |
| 🔴 超強催化 | 可小倉位5%、停損-3% | 正常評分（不扣分） | ✅ 優先推薦 |

**⚠️ 放寬條件**：
1. 必須是**催化相關產業**（費半大漲 → 只放寬半導體相關）
2. 高漲幅股票倉位上限5%、停損-3%（快進快出）
3. 非催化產業維持原規則（一律不放寬）
...
**🔴 產業分散規則（依催化強度動態調整）** 🆕 v6.1：

| 催化等級 | 單一產業上限 | 最少產業數 | 說明 |
|---------|------------|----------|------|
| ⚪ 一般（費半<1%） | ≤50% | ≥3個 | 原規則，如7檔推薦，單一產業最多3-4檔 |
| 🟡 強催化（費半1-3%） | ≤60% | ≥2個 | 催化產業可多1-2檔 |
| 🔴 超強催化（費半>3%） | ≤70% | ≥2個 | 催化產業可佔多數，如7檔中5檔半導體 |

**⚠️ 重要**：放寬上限僅適用於**催化相關產業**，非催化產業仍≤50%
**違反處理**：超過上限必須從其他產業補充推薦

**範例**：
```
⚪ 一般日：推薦7檔，半導體最多3-4檔
🟡 強催化日（費半+2%）：推薦7檔，半導體最多4檔
🔴 超強催化日（費半+4%）：推薦7檔，半導體最多5檔

❌ 仍然錯誤（超強催化日）：推薦7檔，7檔全是半導體（100%）
✅ 正確（超強催化日）：推薦7檔，半導體5檔+金融1檔+其他1檔
```
...
**核心規則提醒**：
- ✅ **雙入口並行**（🆕 v6.0：法人入口 + 時事入口合併，不再只靠 TOP30）
...
**Step 2.5: 🔥 回檔股深度驗證（強制）** 🆕 2026-01-23血淚教訓

**⚠️ 血淚教訓（2026-01-23）**：
- 問題：力成回檔-4.76%，盤中建議「可加碼」
- 盲點：只看表面（回檔+縮量），沒查昨日法人力道變化
- 結果：深度分析才發現動能減弱-67.8% + Level 1預警
- 用戶質疑：「盤中為什麼還是推薦我？」「有照流程走嗎？」

**🔴 強制執行（當準備建議「回檔可加碼」時）**：

對每檔回檔股票，**必須**執行以下4步驗證，**全部通過**才能建議加碼：

```bash
# Step 1: 查詢昨日法人數據
python3 scripts/check_institutional.py [股票代號] [昨日日期YYYYMMDD]

# Step 2: 對比前日法人力道
python3 scripts/check_institutional.py [股票代號] [前日日期YYYYMMDD]
...
| 檢查項目 | 通過條件 | 失敗條件 → 改為「觀望」 |
|---------|---------|----------------------|
| 昨日vs前日法人 | 力道持平或增強 | 🔴 減少>50% → 觀望 |
| 籌碼動能變化 | 動能持平或增強 | 🔴 減弱>30% → 觀望 |
| 反轉預警等級 | 無預警或低預警 | 🔴 Level 1預警 → 觀望 |
| 價格位階 | <70% | 🔴 >80% → 觀望 |
...
結論：❌ 不建議加碼，改為「觀望等待盤後法人數據」
```

**❌ 絕對禁止**：
1. ❌ 只看「回檔+縮量」就建議加碼
2. ❌ 沿用盤前「連3日+10K」判斷，不查昨日實際力道
3. ❌ 跳過chip_analysis.py和reversal_alert.py
4. ❌ 法人力道減少>50%還建議加碼
5. ❌ Level 1預警還建議加碼

**⚠️ 如果4步驗證有任何1項失敗**：
- 改為「觀望等待」
- 標註原因：「昨日法人力道減弱XX%」或「Level 1預警」
- 建議等待盤後法人數據確認
...
```
總推薦：6-8檔（不變）
├─ 盤前核心：4-5檔（直接可執行，盤前即確定進場策略）
└─ 盤中條件單：2-3檔（盤前寫好觸發條件，盤中符合才執行）
...
```markdown
📋 盤中條件單 #1：南亞科(2408)
- 觸發條件：盤中量比 >1.5x 且 漲幅 <5%
- 進場策略：10% 倉位、停損 -3%
- 失效條件：漲幅 >8% 或 量比 <1.0x
- 有效時間：12:30-13:00 尾盤
```

**tracking.json 新增 phase 欄位**：
```json
{
  "stock_code": "2408",
  "stock_name": "南亞科",
  "phase": "conditional",   // "core" = 盤前核心, "conditional" = 盤中條件單
  "trigger_condition": "量比>1.5x 且 漲幅<5%",
...
**最後更新**：2026-01-28
**版本**：v6.1（催化強度動態規則 + 兩階段推薦）🔥
**重大更新**：
- 🆕 **v6.1 催化強度動態規則 + 兩階段推薦**（2026-01-28）
  - 問題：費半+4.14%（超強催化），記憶體股南亞科+6.86%、華邦電+9.96%、旺宏+9.97%，但盤前準確率僅43%
  - 根本原因：催化劑越強，固定規則越不適用（「5日漲幅>10%絕對不推薦」排除了最佳機會）
  - 解決1：**催化劑強度分級**（一般/強催化/超強催化）→ 產業集中上限和排除門檻動態調整
    - 一般（費半<1%）：原規則（產業≤50%、>10%不推薦）
    - 強催化（費半1-3%）：催化產業≤60%、>10%標註風險但不排除
    - 超強催化（費半>3%）：催化產業≤70%、>10%可小倉位5%
  - 解決2：**兩階段推薦**（盤前核心4-5檔 + 盤中條件單2-3檔）
    - 盤前核心：直接可執行的推薦
    - 盤中條件單：寫好觸發條件（量比、漲幅），盤中符合才執行
    - tracking.json 新增 `phase` 欄位（`"core"` / `"conditional"`）
  - 效果：超強催化日不再系統性排除催化產業高漲幅股票
  - 教訓：催化越強 → 規則越應放寬，盤前不用全部決定完
- 🆕 **v6.0 雙入口選股流程**（2026-01-28）
  - 問題：費半+4.14%但記憶體股南亞科(2408)完全沒被掃到
  - 原因：只靠法人 TOP30 入口，昨日法人數據無法預知昨晚時事催化
  - 解決：新增「入口 B（時事展開）」與原有「入口 A（TOP30）」雙軌並行
  - 流程：
    - 入口 A：TOP30 法人買賣超 → 查時事催化 → 評分
    - 入口 B：今日催化劑 → 展開受惠產業代表股 → 查法人態度 → 評分
    - 合併去重 → 統一五維度排序
  - 新增完整「時事→產業→代表股」對照表（含半導體5次產業、AI伺服器4次產業等）
  - 效果：消除「時事受惠但不在 TOP30」的盲區
  - 教訓：法人數據是昨天的，時事催化是昨晚的，兩個入口缺一不可
- 🆕 **v5.9.3 盤中回檔股深度驗證**（2026-01-23）
  - 問題：力成回檔-4.76%，盤中建議「可加碼」但深度分析發現動能減弱-67.8% + Level 1預警
  - 原因：盤中分析只看表面（回檔+縮量+盤前連3日買超），沒查昨日法人力道、沒執行chip_analysis、沒執行reversal_alert
  - 用戶質疑：「盤中為什麼還是推薦我？」「有照流程走嗎？」「為什麼要一直提醒？」
  - 解決：新增 Step 2.5 回檔股深度驗證強制流程
  - 功能：
    - 查詢昨日vs前日法人力道變化（減少>50% → 觀望）
    - 執行chip_analysis.py籌碼動能分析（減弱>30% → 觀望）
    - 執行reversal_alert.py反轉預警檢查（Level 1預警 → 觀望）
    - 查詢價格位階（>80% → 觀望）
  - 判斷標準：4項檢查必須全部通過才能建議加碼
  - 效果：避免在籌碼動能減弱時錯誤建議加碼
  - 教訓：回檔≠機會，必須深度驗證籌碼動能
- 🆕 **v5.9.2 法人反轉多層次預警**（2026-01-22）
  - 問題：凱基金 1/19 +35K → 1/21 反轉-2.5K（沒有提前預警）
  - 解決：reversal_alert.py v2.0 四層預警系統
  - 功能：
    - Level 1 ⚠️：動能減速>30%（早期預警，考慮減碼）
    - Level 2 ⚠️⚠️：連買後突然賣（密切觀察，準備停損）
    - Level 3 🔴：連續2日賣超（確認反轉，建議出場）
    - Level 4 🔴🔴：爆量賣超>20K（極度危險，立即出場）
  - 整合：chip_analysis.py 的動能分析
  - 範例：
    - 凱基金：Level 2（買轉賣，注意反轉）
    - 廣達：Level 1（動能減速-69.2%，早期預警）
  - 效果：提前偵測法人翻臉，避免停損不及時
  - 用法：`python3 scripts/reversal_alert.py 2883 2303`
- 🆕 **v5.9.1 籌碼動能分析**（2026-01-22）
  - 問題：陽明買超+21K但-0.36%（只看「有買超」看不出「買超減弱」）
  - 解決：chip_analysis.py 加入動能分析（前5日 vs 近5日平均）
  - 功能：
    - 計算買超動能變化率
    - 判斷「加速買超」vs「減速買超」
    - 動能等級：⭐⭐⭐ 加速（>+50%）/ ⭐⭐ 增強 / ⭐ 持續 / ⚠️ 減弱 / 🔴 大幅減弱
  - 範例：
    - 聯電：動能 +110.5% → ⭐⭐⭐ 加速買超（強力推薦）
    - 凱基金：動能 +16.4% → ⭐ 持續買超（穩健推薦）
