...
**Step 1.5: 參考 PREDICTION 歷史教訓（強制）** 🆕
```bash
# 查看長期績效追蹤和歷史教訓
cat docs/PREDICTION_ACCURACY_TRACKING.md | head -200
```
...
**Step 3.5: 🔥 找「正在佈局」的股票（關鍵！）** 🆕
```bash
# 查詢連續多日法人買超數據（至少3日）
# 篩選條件：連續3日以上買超 + 5日漲幅<3%
```

**🔥 佈局股篩選邏輯（2025-12-11新增）**：

**❌ 舊方法（錯誤）**：
- 看昨天法人買超 TOP10 → 推薦 → **股價已經漲了** → 用戶接盤

**✅ 新方法（正確）**：
- 看**連續3-5日**法人買超趨勢
- 篩選「累計買超大」但「股價還沒漲（<3%）」的股票
- 這才是法人**正在佈局**的訊號
...
| 累計買超量 | >500萬張 | 有實質力道 |
| 5日漲幅 | <3% | 股價還沒反映 |
| 最佳訊號 | 5日漲幅<0% | 法人買但股價跌=悄悄吸貨 |

**佈局股優先順序**：
1. ⭐ **連續買超+股價下跌** = 最佳佈局（法人悄悄吸貨）
2. 🟡 **連續買超+股價微漲<2%** = 佈局中
3. ⚠️ **連續買超+股價漲>5%** = 已反映，追高風險

**範例輸出**：
```
🔍 法人「正在佈局」的股票（連續買超+還沒漲）
代號   連續買超   累計買超        5日漲幅   狀態
2382   3日      +17,867K        -1.2%    ✅ 佈局中（還沒漲）
2303   3日      +85,207K        +1.7%    ✅ 佈局中（微漲）
2887   3日      +66,643K        +0.8%    ✅ 佈局中（微漲）
3037   3日      +37,949K        +17.4%   ⚠️ 已漲（不推薦）
```
...
**Step 0.5: 參考 PREDICTION 歷史教訓（強制）** 🆕
```bash
# 快速查看歷史教訓，用於盤中異常判斷
grep -A 5 "重大教訓" docs/PREDICTION_ACCURACY_TRACKING.md
```
...
2. **遺漏機會根本原因** - 法人數據盲點？產業邏輯不足？評分權重問題？
...
**Step 5: 更新 PREDICTION_TRACKING（第五優先）** 🆕
```bash
# 更新長期績效追蹤
# 檔案位置：docs/PREDICTION_ACCURACY_TRACKING.md
```

**🔥 PREDICTION_TRACKING 強制更新內容**：
1. **新增當日記錄** - 推薦股票、收盤價、漲跌%、結果
2. **準確率統計** - X/Y = Z%
3. **重大教訓記錄** - 如有失敗模式需記錄（如12/10法人反轉）
4. **更新趨勢圖** - 準確率趨勢變化
5. **更新核心發現** - 如有新規則需新增

**格式範例**：
```markdown
### 2025/12/10 盤前推薦 → 當日收盤驗證
...
**最後更新**：2025-12-09
**版本**：v5.1（個人持股分析工具+出場機制設計）
**重大更新**：
- 🆕 **新增個人持股分析工具**：`my_holdings_analyzer.py` 獨立運行
- 🆕 **四維度持股評分**：即時價格+法人數據+時事分析+產業邏輯
- 🆕 **健康度評分系統**：100分制評分+操作建議+風險警示
- 🆕 **出場機制設計**：三層出場機制（盤前健康檢查+盤中動態停損+盤後綜合評估）
- 🔧 **國際市場數據工具**：fetch_us_asia_markets.py 完善版
