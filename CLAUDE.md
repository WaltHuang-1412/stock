...
```bash
# 自動識別今日熱點產業
python3 scripts/identify_hotspots.py

# 輸出範例（強勢日）：
# 🔥 最終熱點：2 個產業
# 1. 🔴 AI/人工智慧 (AI)
#    催化劑：輝達+5.87%
#    強度：strong → 建議展開深度 Tier 0-3

# 輸出範例（偏空日）：
# 🔥 最終熱點：1 個產業
# 1. ⚪ 航空運輸 (航空)
#    催化劑：油價-2.68%（利多航空）
#    強度：weak → 建議展開深度 Tier 0-1
```

**識別邏輯**：
- 美股數據觸發：輝達、費半、油價、Micron 等
- 台股新聞觸發：關鍵字匹配 20 個產業
- 自動判斷強度：strong（>5%）/ medium（2-5%）/ weak（<2%）

##### B-2：產業鏈展開
```bash
# 根據熱點動態展開產業鏈
python3 scripts/dynamic_industry_expander.py

# 輸出：30-50 檔候選（依催化強度而定）
```

**展開深度規則**：
| 催化強度 | 展開深度 | 候選數 | 案例 |
|---------|---------|--------|------|
| **strong** | Tier 0-3 | 40-50檔 | 輝達+5.87% → AI全產業鏈 |
| **medium** | Tier 0-2 | 20-30檔 | 費半+2% → 半導體核心鏈 |
| **weak** | Tier 0-1 | 3-10檔 | 油價-2.68% → 航空核心股 |
| **無熱點** | - | 0檔 | 市場偏空 → 依賴軌道A |
