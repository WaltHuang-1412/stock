...
### 🔴 7. 動能計算誤判災難（2026-05-22）

**問題**：
- 景碩(3189) 和 康舒(6282) 都被錯誤給予 Module A L3 +15分
- 實際上兩者都不在 `catalyst_preposition_scan.json` 的輸出中

**根本原因**：
- 看到 chip_analysis 的「大量單日買超」就誤以為是「佈局完成」
- chip_analysis 的「佈局完成⭐⭐⭐⭐⭐」標籤 ≠ Module A L3 資格
- 正確判斷必須計算動能方向：

```
動能變化 = (近5日平均 - 前5日平均) / |前5日平均| × 100

- 近5日 < 前5日 → 動能遞減 = 佈局完成（法人大量進貨後逐漸減少）
- 近5日 > 前5日 → 動能爆發 = 追高排除
```

**景碩 05-22 實際動能**：
- 近5日(05/15-05/21)平均 = +2,228/日，前5日(05/08-05/14)平均 = +972/日
- 動能變化 = +129.2% → 動能爆發 → **應排除，不應給 +15 分**

**康舒 05-22 實際動能**：
- 近5日平均 = +4,472/日，前5日平均 = +3,058/日
- 動能變化 = +46.3% → 正常區間（0%~+50%）→ **0分，不應給 +15 分**

**教訓**：
- ❌ **不得憑 chip_analysis 標籤（佈局完成/動能爆發）判斷 Module A 等級**
- ❌ **不得憑單日大量買超判斷為 L3 +15**
- ✅ **Module A 加分唯一依據：`catalyst_preposition_scan.json` 中的實際輸出**
- ✅ **若股票不在 catalyst_preposition_scan.json 輸出 → Module A = 0分，無論 chip_analysis 如何顯示**

**正確流程**：
```
1. 執行 catalyst_preposition_scan.py 產生 JSON
2. 查詢該股票是否出現在 JSON 輸出中
3. 有→讀取 level/momentum_pct 欄位判定等級
4. 無→Module A = 0（不加分、不扣分）
```

**受影響評分**（已結算，僅記錄）：
- 景碩 3189（2026-05-22）：score 誤報 110，正確應為 ~95（超強催化覆寫後）
- 康舒 6282（2026-05-22）：score 誤報 102，正確應為 ~87

---

### 🔴 8. T86 快取靜默污染（2026-05-26）

**問題**：
- `twse_institutional_cache.py` 從 TWSE API 取得資料，但 API 在非交易日
  靜默回傳最後一個交易日的數據（不報錯，正常 200 回應）
- 導致 2026-05-26 T86 快取存入 2026-05-18 的資料
- `catalyst_preposition_scan.py` 誤判 6451 訊芯-KY 為 L3（實為 L2）

**解決方案（commit 8a59b92）**：
- `twse_institutional_cache.py` 加入日期驗證：
  若 API 回傳日期 ≠ 請求日期 → 快取寫入失敗、拋出警告

**教訓**：
- ❌ **不得假設外部 API 在非交易日回傳的數據日期是正確的**
- ✅ **所有快取寫入前必須驗證回傳數據的日期欄位**
